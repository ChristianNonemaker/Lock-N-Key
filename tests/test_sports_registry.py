...
def test_registry_rejects_unknown_or_disabled_collection():
    with pytest.raises(ValueError):
        validate_schedule_sports(["Basketball_NCAAB"])
    with pytest.raises(ValueError):
        validate_schedule_sports(["basketball_nba"])
    with pytest.raises(ValueError):
        validate_odds_sports(["soccer_epl"])
    with pytest.raises(ValueError):
        active_schedule_sports("basketball_nba")
