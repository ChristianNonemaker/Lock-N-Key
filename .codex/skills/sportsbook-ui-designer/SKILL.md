...
Improve in this order unless the user says otherwise:

1. `NCAAB View`: first impression and daily workflow.
2. `Game Browser`: fast game discovery and line scan.
3. `Game Detail`: movement, splits, snapshots, outcomes, and CLV clarity.
4. `Model Panel`: honest signal display and missing-artifact guidance.
5. `Pipeline Status`: trust, freshness, and operations.
6. `Backtest Dashboard`: filters and clear separation of exploratory and validated metrics.
...
- Tests when API/data contracts change: `pytest tests/ -v`
- Screenshot checks when layout changes: desktop and mobile, populated and empty states.

If screenshots cannot be captured, say why and inspect the layout code carefully instead.
